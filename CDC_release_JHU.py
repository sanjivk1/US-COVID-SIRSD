...
def CDC(state, reopen_date, ax3):
	...
	fig.savefig(f'JHU/CDC/CDC_{state}.png', bbox_inches="tight")
	# plt.show()
	plt.close(fig)
...
def CDC_all():
	...
	fig.savefig(f'JHU/CDC/CDC_grid.png', bbox_inches="tight")
	plt.close(fig)
...
def delay_all():
	if not os.path.exists('JHU/CDC/delay'):
		os.makedirs('JHU/CDC/delay')
	fig = plt.figure(figsize=(14, 18))
	col = 4
	row = 6
	i = 0
	...
	fig.savefig('JHU/CDC/delay/delay_grid.png', bbox_inches="tight")

	print('average ratio =', np.mean([row[1] for row in table]))
	out_df = pd.DataFrame(table, columns=['State - County', 'Ratio'])
	out_df.to_csv('JHU/CDC/delay/improvement.csv', index=False)
