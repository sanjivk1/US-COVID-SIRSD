...
def save_para_combined(state, paras):
	...
	df.to_csv(f'JHU/combined2W_{end_date}/{state}/para.csv', index=False, header=True)
...
def plot_combined(state, confirmed, death, hosp, days, n_0, reopen_day, para):
	...
	ax3.legend()
	fig.autofmt_xdate()
	fig.savefig(f'JHU/combined2W_{end_date}/{state}/sim.png', bbox_inches="tight")
...
def CI_state_combined(state, ConfirmFile, DeathFile, PopFile, dates):
	...
	ParaFile = f'JHU/combined2W_{end_date}/{state}/para.csv'
	t1 = time.perf_counter()

	# if not os.path.exists(f'combined2W_{end_date}/{state}'):
	# 	os.makedirs(f'combined2W_{end_date}/{state}')
	if not os.path.exists(f'JHU/CI_{end_date}/{state}'):
		os.makedirs(f'JHU/CI_{end_date}/{state}')
	...
	out_df.to_csv(f'JHU/CI_{end_date}/{state}/paraCI.csv', index=False)
	t2 = time.perf_counter()
	print(f'{round(t2 - t1, 3)} seconds in total for {state}\n')
...
def interval(state, reopen_date, ax):
	...
	CIFile = f'JHU/CI_{end_date}/{state}/paraCI.csv'
	...
	df.loc[len(df)] = D_max
	df.loc[len(df)] = D_min
	df.to_csv(f'JHU/CI_{end_date}/{state}/GD_high_low.csv', index=False)
	return
...
def CI_para(state):
	df = pd.read_csv(f'JHU/combined2W_{end_date}/{state}/para.csv')
	beta = df['beta'].iloc[0]
	gamma = df['gamma'].iloc[0]
	gamma2 = df['gamma2'].iloc[0]
	a2 = df['a2'].iloc[0]

	df2 = pd.read_csv(f'JHU/CI_{end_date}/{state}/paraCI.csv')
...
def CI_para_all():
	...
	out_df.to_csv(f'JHU/CI_{end_date}/para_table.csv', index=False)
	# print(table)
	return
