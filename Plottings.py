...
def plot_GD(state, ax, folder):
	...
	# read reported data
	df_G_data = pd.read_csv(f'data/Confirmed-counties.csv')
	G_data = df_G_data[df_G_data['county'] == state].loc[:, days_init[0]: days_reopen[-1]]
	G_data = list(G_data.iloc[0])
	df_D_data = pd.read_csv(f'data/Death-counties.csv')
...
def plot_grid(folder):
	...
	states = ['AZ-Maricopa', 'CA-Los Angeles', 'CA-Riverside', 'FL-Broward', 'FL-Miami-Dade',
	          'GA-Fulton', 'IL-Cook', 'LA-Jefferson', 'MA-Middlesex', 'MD-Prince Georges', 'MN-Hennepin',
	          'NC-Mecklenburg', 'NJ-Bergen', 'NJ-Hudson', 'NV-Clark', 'NY-New York', 'OH-Franklin', 'PA-Philadelphia',
	          'TN-Shelby', 'TX-Dallas', 'TX-Harris--Houston', 'UT-Salt Lake', 'VA-Fairfax', 'WI-Milwaukee']
	# states = ['IL-Cook', 'TX-Harris--Houston']
...
def plot_state_combined(ax, state, ConfirmFile, DeathFile, PopFile, dates):
	reopen_date = dates[0]
	HospFile = f'data/hosp/{state}/Top_Counties_by_Cases_Full_Data_data.csv'
	...
	# read simulation
	SimFile = f'combined2W_{end_date}/{state}/sim.csv'
...
def plot_combined():
	...
	plot_state_combined(ax, state, 'data/Confirmed-counties.csv', 'data/Death-counties.csv',
	                    'data/CountyPopulation.csv', dates)
	...
	ax2 = plot_state_combined(ax, state, 'data/Confirmed-counties.csv', 'data/Death-counties.csv',
	                          'data/CountyPopulation.csv', dates)
	...
	# fig.autofmt_xdate()
	fig.savefig(f'combined2W_{end_date}/grid.png', bbox_inches="tight")
	# plt.show()
	plt.close(fig)
...
def plot_bar():
	...
	state = 'NY-New York'
	dates = ['2020-06-22', end_date]
	point = read_bar(state, f'combined2W_{end_date}/{state}', 'data/Confirmed-counties.csv',
	                 'data/CountyPopulation.csv', dates)
	points.append(point)
	...
	for i in range(len(points)):
		state = points[i][-1]
		ax2.bar3d(points[i][0], points[i][1], 0, 0.02, 0.02, points[i][3], alpha=0.6,
		          color=color_dict.setdefault(state, 'black'))
	fig.savefig('3D/3D.png', bbox_inches="tight")
	plt.close(fig)
	...
	for i in range(len(points)):
		state = points[i][-1]
		ax.bar3d(points[i][0], points[i][1], 0, 0.02, 0.02, math.log(points[i][3]), alpha=0.6,
		         color=color_dict.setdefault(state, 'black'))
	xl, xh = ax.get_xlim()
	yl, yh = ax.get_ylim()
	...
	fig.savefig('3D/3D_log.png', bbox_inches="tight")
	# plt.show()
	plt.close(fig)
	...
	for i in range(len(points)):
		state = points[i][-1]
		ax.scatter(points[i][1], math.log(points[i][3]), color=color_dict.setdefault(state, 'black'))

	X = [points[i][1] for i in range(len(points))]
	...
	ax.set_xlabel('SLR')
	ax.set_ylabel('log(PIR)')
	fig.savefig('3D/log_S_left.png', bbox_inches="tight")
	# plt.show()
	plt.close(fig)
	...
	for i in range(len(points)):
		state = points[i][-1]
		ax.scatter(points[i][0], math.log(points[i][3]), color=color_dict.setdefault(state, 'black'))
	...
	ax.set_xlabel('PAR')
	ax.set_ylabel('log(PIR)')
	fig.savefig('3D/log_I.png', bbox_inches="tight")
	# plt.show()
	plt.close(fig)
...
def fit_SIR_MT(state, SimFolder, ConfirmFile, PopFile, dates):
	if not os.path.exists(f'SIR/{state}'):
		os.makedirs(f'SIR/{state}')
	...
	df.to_csv(f'SIR/{state}/sim.csv', index=False)
	...
	df.to_csv(f'SIR/{state}/para.csv', index=False)
	...
	fig.autofmt_xdate()
	ax.legend()
	fig.savefig(f'SIR/{state}/fit.png', bbox_inches="tight")
	plt.close(fig)
	# plt.show()
...
def fit_all_SIR():
	state = 'NY-New York'
	dates = ['2020-06-22', end_date]
	fit_SIR_MT(state, f'combined2W_{end_date}/{state}', 'data/Confirmed-counties.csv',
	           'data/CountyPopulation.csv', dates)
...
def compare_state(state, out_df, ax):
	...
	df = pd.read_csv(ConfirmFile)
	confirmed = df[df.iloc[:, 0] == state]
	DeathFile = 'data/Death-counties.csv'
	df2 = pd.read_csv(DeathFile)
	death = df2[df2.iloc[:, 0] == state]
	...
	df3 = pd.read_csv(f'init_only_{end_date}/{state}/sim.csv')
	...
	ax2.legend()
	fig.autofmt_xdate()
	fig.savefig(f'comparison/comparison_{state}.png', bbox_inches="tight")
	plt.close(fig)
	return
...
def compare_all():
	...
	fig.savefig('comparison/comparison.png', bbox_inches="tight")

	out_df = pd.DataFrame(out_df,
	                      columns=['state', 'RMSE_G', 'RMSE_D', 'RMSE_wtd_G', 'RMSE_wtd_D', 'RMSE_G2', 'RMSE_SIR',
	                               'R2_G', 'R2_D', 'R2_wtd_G', 'R2_wtd,D'])
	out_df.to_csv('comparison/RMSE.csv', index=False)
	# print(out_df)
	return
...
def interval_GD(state, reopen_date, ax):
	...
	# read interval
	IntervalFile = f'CI_{end_date}/{state}/GD_high_low.csv'
...
def interval_GD_all():
	...
	fig.subplots_adjust(hspace=0.4, wspace=0.35)
	plt.rcParams.update({'font.size': 10})
	fig.savefig(f'CI_{end_date}/grid.png', bbox_inches="tight")
	# plt.show()
	plt.close(fig)
	return
...
def peak_by_H():
	...
	for i in range(len(points)):
		state = points[i][-1]
		ax.bar3d(points[i][0], points[i][4], 0, 0.02, 0.1, math.log(points[i][3]), alpha=0.6,
		         color=color_dict.setdefault(state, 'black'))
	xl, xh = ax.get_xlim()
	yl, yh = ax.get_ylim()
	xs = np.linspace(xl, xh, 100)
	...
	ax.set_ylim(yl, yh)
	ax.view_init(elev=5, azim=315)
	fig.savefig('3D/3D_H.png', bbox_inches="tight")
	# plt.show()
	plt.close(fig)
	...
	for i in range(len(points)):
		state = points[i][-1]
		ax.scatter(points[i][4], np.log(points[i][3]), color=color_dict.setdefault(state, 'black'))
		if points[i][4] > 4:
			print(state)

	X = [points[i][4] for i in range(len(points))]
	Y = [np.log(points[i][3]) for i in range(len(points))]
	C = poly.polyfit(X, Y, 1)
	X2 = np.arange(min(X), max(X), (max(X) - min(X)) / 100)
	Y2 = poly.polyval(X2, C)
	...
	ax.set_xlabel('HSR')
	ax.set_ylabel('log(PIR)')
	fig.savefig('3D/log_H.png', bbox_inches="tight")
	# plt.show()
	plt.close(fig)
