...
c1_range = (0.8, 1)
h_range = (1 / 30, 1 / 14)
Hiding_init_range = (0, 10)
k_range = (0.1, 2)
k2_range = (0.1, 2)
E_initial_range = (0, 1)
I_initial_range = (0, 1)
start_date = '2021-02-01'
reopen_date = '2021-06-10'
# end_date = '2021-06-08'
# end_date = '2021-06-15'
# end_date = '2021-06-22'
vac_date = '2021-02-01'
...
def simulate_vac(size, S, E, I, A, IH, IN, D, R, G, H, beta, gammaE, alpha, gamma, gamma2, gamma3, a1, a2, a3, h,
                 Hiding_init, eta, c1, n_0, reopen_day, vaccine_day, vaccine_speeds, vac_period1, vac_period2, vac_eff1,
                 vac_eff2):
    result = True
...
def loss_init(point, c1, confirmed, death, n_0, vaccine_day, vaccine_speeds):
    size = len(confirmed)
    beta = point[0]
    gammaE = point[1]
    alpha = point[2]
    ...
    result, [S, E, I, A, IH, IN, D, R, G, H,
             ...
             betas] = simulate_vac(size, S, E, I, A, IH, IN, D, R, G, H, beta, gammaE, alpha, gamma, gamma2, gamma3, a1,
                                   a2, a3, 1, 0, eta, c1, n_0, size, vaccine_day, vaccine_speeds, v_period1, v_period2,
                                   v_eff1, v_eff2)
...
def loss_reopen(point, confirmed, death, n_0, vaccine_day, vaccine_speeds, reopen_day, para_init):
    size = len(confirmed)
    h = point[0]
    Hiding_init = point[1]
    ...
    result, [S, E, I, A, IH, IN, D, R, G, H,
        ...
        simulate_vac(size, S, E, I, A, IH, IN, D, R, G, H, beta, gammaE, alpha, gamma, gamma2, gamma3, a1, a2, a3, h,
                     Hiding_init, eta, c1, n_0, reopen_day, vaccine_day, vaccine_speeds, v_period1, v_period2, v_eff1,
                     v_eff2)
...
def fit_init(confirmed0, death0, vaccine_day, vaccine_speeds, n_0):
    np.random.seed()
    ...
    for c1 in np.arange(c1_range[0], c1_range[1], 0.01):
        ...
        optimal = minimize(loss_init, [uni(beta_range[0], beta_range[1]),
                           ...
                           args=(c1, confirmed, death, n_0, vaccine_day, vaccine_speeds), method='L-BFGS-B',
                           bounds=[beta_range,
                                   gammaE_range,
                                   alpha_range,
                                   gamma_range,
                                   ...
                                   E_initial_range,
                                   I_initial_range])
        current_loss = loss_init(optimal.x, c1, confirmed, death, n_0, vaccine_day, vaccine_speeds)
    ...
    result, [S, E, I, A, IH, IN, D, R, G, H,
        ...
        simulate_vac(size, S, E, I, A, IH, IN, D, R, G, H, beta, gammaE, alpha, gamma, gamma2, gamma3, a1, a2, a3, 1, 0,
                     eta, c1, n_0, size, vaccine_day, vaccine_speeds, v_period1, v_period2, v_eff1, v_eff2)
...
def fit_reopen(confirmed0, death0, vaccine_day, vaccine_speeds, reopen_day_gov, n_0, para_init):
    np.random.seed()
    confirmed = confirmed0.copy()
    death = death0.copy()
    size = len(confirmed)
    ...
    c_max = 0
    min_loss = 10000
    for reopen_day in range(reopen_day_gov, min(reopen_day_gov + 22, size + 1)):
        optimal = minimize(loss_reopen, [uni(h_range[0], h_range[1]),
                                         # uni(Hiding_init_range[0], Hiding_init_range[1])],
                                         Hiding_init_range[0]],
                           args=(confirmed, death, n_0, vaccine_day, vaccine_speeds, reopen_day, para_init),
                           method='L-BFGS-B',
                           bounds=[h_range, Hiding_init_range])
        current_loss = loss_reopen(optimal.x, confirmed, death, n_0, vaccine_day, vaccine_speeds, reopen_day, para_init)
        if current_loss < min_loss:
            # print(f'updating loss={current_loss} with c1={c1}')
            min_loss = current_loss
            h = optimal.x[0]
...
def MT_fitting_init(state, confirmed, death, n_0, vaccine_day, vaccine_speeds):
    para_best = []
    min_loss = 10000
    with concurrent.futures.ProcessPoolExecutor() as executor:
        # t1 = time.perf_counter()
        results = [executor.submit(fit_init, confirmed, death, vaccine_day, vaccine_speeds, n_0) for _ in
                   range(num_threads)]
...
def MT_fitting_reopen(state, confirmed, death, n_0, vaccine_day, vaccine_speeds, reopen_day_gov, para_init):
    para_best = []
    min_loss = 10000
    with concurrent.futures.ProcessPoolExecutor() as executor:
        # t1 = time.perf_counter()
        results = [
            executor.submit(fit_reopen, confirmed, death, vaccine_day, vaccine_speeds, reopen_day_gov, n_0, para_init)
            for _ in
            range(num_threads)]
...
def fit_state_split(state, ConfirmFile, DeathFile, PopFile, end_date):
    t1 = time.perf_counter()
    state_path = f'india/fitting_split_{end_date}_{reopen_date}/{state}'
    ...
    vaccine_speeds = [daily_vspeed] * len(days_full)

    # initial fitting
    para_init = MT_fitting_init(state, confirmed_init, death_init, n_0, vaccine_day, vaccine_speeds)

    # reopen fitting
    para_reopen = MT_fitting_reopen(state, confirmed_full, death_full, n_0, vaccine_day, vaccine_speeds, reopen_day_gov,
                                    para_init)
    ...
    [S, E, I, A, IH, IN, D, R, G, H,
     S0, E0, I0, A0, IH0, IN0, D0, R0, G0, H0, S1, E1, I1, A1, IH1, IN1, D1, R1, G1, H1,
     S2, E2, I2, A2, IH2, IN2, D2, R2, G2, H2, S3, E3, I3, A3, IH3, IN3, D3, R3, G3, H3, betas] = \
        plot_full(state, confirmed_full, death_full, days_full, n_0, para, state_path, vaccine_day, vaccine_speeds)
...
def plot_full(state, confirmed, death, days, n_0, para, state_path, vaccine_day, vaccine_speeds):
...
def extend_state_split(state, ConfirmFile, DeathFile, PopFile, ParaFile, sim_enddate, data_enddate):
    state_path = f'India/extended_split_{sim_enddate}/{state}'
    if not os.path.exists(state_path):
        os.makedirs(state_path)
    ...
    vaccine_speeds = [daily_vspeed] * size
...
def extend_all_split(sim_enddate):
    ...
    data_enddate = '2021-07-16'
    sim_releasedate = '2021-06-10'
    sim_folder = f'india/fitting_split_{sim_enddate}_{sim_releasedate}'
    # with concurrent.futures.ProcessPoolExecutor() as executor:
    ...
    fig.autofmt_xdate()
    fig.savefig(f'india/extended_split_{sim_enddate}/India_ext.png', bbox_inches="tight")
    plt.close(fig)
...
def save_para_all_vac(end_date):
    fitting_folder = f'Inida/fitting_{end_date}'
    out_table = []
    for state in states:
...
def main():
    # fit_all_split('2021-07-16')
    # fit_all_split('2021-06-15')
    # fit_all_split('2021-06-22')
    # save_para_all_vac()
    extend_all_split('2021-07-16')
