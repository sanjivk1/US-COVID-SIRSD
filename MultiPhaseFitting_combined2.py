...
def save_para_combined(state, paras):
	...
	df.to_csv(f'combined2W_{end_date}/{state}/para.csv', index=False, header=True)
...
def loss_combined(point, c1, confirmed, death, hosp, n_0, SIRG, reopen_day):
	...
	return -(0.45 * metric0 + 0.45 * metric1 + 0.1 * metric2)
...
def fit_state_combined(state, ConfirmFile, DeathFile, PopFile, dates):
	...
	csv_file = f'combined2W_{end_date}/{state}/sim.csv'
...
def plot_combined(state, confirmed, death, hosp, days, n_0, reopen_day, para):
	...
	ax3.legend()
	fig.autofmt_xdate()
	fig.savefig(f'combined2W_{end_date}/{state}/sim.png', bbox_inches="tight")
...
def CI_state_combined(state, ConfirmFile, DeathFile, PopFile, dates):
	...
	ParaFile = f'combined2W_{end_date}/{state}/para.csv'
	t1 = time.perf_counter()

	# if not os.path.exists(f'combined2W_{end_date}/{state}'):
	# 	os.makedirs(f'combined2W_{end_date}/{state}')
	if not os.path.exists(f'CI_{end_date}/{state}'):
		os.makedirs(f'CI_{end_date}/{state}')
	...
	out_df.to_csv(f'CI_{end_date}/{state}/paraCI.csv', index=False)
	t2 = time.perf_counter()
	print(f'{round(t2 - t1, 3)} seconds in total for {state}\n')
...
def CI_all_combined():
	...
	state = 'NY-New York'
	dates = ['2020-06-22', end_date]
	CI_state_combined(state, 'data/Confirmed-counties.csv', 'data/Death-counties.csv', 'data/CountyPopulation.csv',
	                  dates)
...
def hist(state):
	...
	df = pd.read_csv(f'CI_{end_date}/{state}/paraCI.csv')
	...
	fig.suptitle(state)
	fig.savefig(f'CI_{end_date}/{state}/hist.png', bbox_inches='tight')
	# plt.show()
	plt.close(fig)
...
def interval(state, reopen_date, ax):
	...
	CIFile = f'CI_{end_date}/{state}/paraCI.csv'
	...
	for i in range(len(days)):
		...
		D_current.sort()
		size = len(G_current)
		G_max.append(G_current[round(size * 0.975) + 1])
		G_min.append(G_current[round(size * 0.025) + 1])
		D_max.append(D_current[round(size * 0.975) + 1])
		D_min.append(D_current[round(size * 0.025) + 1])
	...
	ax.fill_between(days, [i / 1000 for i in G_min], [i / 1000 for i in G_max], alpha=0.3)
	ax.legend()
	ax.set_title(state)
	...
	df.loc[len(df)] = D_max
	df.loc[len(df)] = D_min
	df.to_csv(f'CI_{end_date}/{state}/GD_high_low.csv', index=False)
	return
...
def CI_para(state):
	df = pd.read_csv(f'combined2W_{end_date}/{state}/para.csv')
	beta = df['beta'].iloc[0]
	gamma = df['gamma'].iloc[0]
	gamma2 = df['gamma2'].iloc[0]
	a2 = df['a2'].iloc[0]

	df2 = pd.read_csv(f'CI_{end_date}/{state}/paraCI.csv')
	...
	row = [state]
	row.append(
		f'{round(beta, 3)}, ({round(betas[round(size * 0.025) - 1], 3)}, {round(betas[round(size * 0.975) - 1], 3)})')
	row.append(
		f'{round(gamma, 3)}, ({round(gammas[round(size * 0.025) - 1], 3)}, {round(gammas[round(size * 0.975) - 1], 3)})')
	row.append(
		f'{round(gamma2, 3)}, ({round(gamma2s[round(size * 0.025) - 1], 3)}, {round(gamma2s[round(size * 0.975) - 1], 3)})')
	row.append(f'{round(a2, 3)}, ({round(a2s[round(size * 0.025) - 1], 3)}, {round(a2s[round(size * 0.975) - 1], 3)})')
	return row
...
def CI_para_all():
	...
	out_df.to_csv(f'CI_{end_date}/para_table.csv', index=False)
	# print(table)
	return
