...
from SIRfunctions import SEIARG_SG
import datetime
...
def save_sim_SG(csv_filename, data, days):
...
def save_para_SG(para_file, paras):
	para_label = ['beta', 'gammaE', 'alpha', 'gamma', 'gamma2', 'gamma3', 'a1', 'a2', 'a3', 'h', 'eta', 'c1', 'metric1', 'metric2', 'metric3', 'r1', 'r2', 'r3']
...
def simulate_SG(size, SIRG, S, E, I, A, IH, IN, D, R, G, H, beta, gammaE, alpha, gamma, gamma2, gamma3, a1, a2,
                a3, h, eta, c1, n_0, H0):
	result = True
	betas = [beta]
	for i in range(1, size):
		...
		delta = SIRG(i,
		             [S[i - 1], E[i - 1], I[i - 1], A[i - 1], IH[i - 1], IN[i - 1], D[i - 1], R[i - 1], G[i - 1], beta,
		              gammaE, alpha, gamma, gamma2, gamma3, a1, a2, a3, eta, n_0, c1, H0])
...
# combined fitting
def loss_SG(point, c1, confirmed, death, hosp, visits, n_0, SIRG):
	size = len(confirmed)
	beta = point[0]
	gammaE = point[1]
	alpha = point[2]
	...
	G = [confirmed[0]]
	H = [0]
	H.extend([v * eta * h for v in np.diff(visits)])
	H0 = eta * h * (max(visits) - visits[0])
	if len(H) != size:
		print('H length wrong!!!')
	result, [S, E, I, A, IH, IN, D, R, G, H, betas] \
		= simulate_SG(size, SIRG, S, E, I, A, IH, IN, D, R, G, H, beta, gammaE, alpha, gamma, gamma2, gamma3,
		              a1, a2, a3, h, eta, c1, n_0, H0)
...
def loss_init(point, c1, confirmed, death, hosp, n_0, SIRG, reopen_day):
	...
	result, [S, I, IH, IN, D, R, G, H, betas] \
		= simulate_SG(size, SIRG, S, I, IH, IN, D, R, G, H, beta, gamma, gamma2, a1, a2, a3, h, k, k2, eta, c1,
		              n_0, reopen_day)
...
def fit_SG(confirmed0, death0, hosp0, visits0, days, n_0, metric1, metric2, metric3):
	np.random.seed()
	...
	for c1 in np.arange(c1_range[0], c1_range[1], 0.01):
		...
		optimal = minimize(loss_SG, [uni(beta_range[0], beta_range[1]),
		                   ...
		                   args=(c1, confirmed, death, hosp, visits, n_0, SEIARG_SG), method='L-BFGS-B',
		                   bounds=[beta_range,
		                           gammaE_range,
		                           alpha_range,
		                           gamma_range,
		                           ...
		                           eta_range,
		                           h_range])
		current_loss = loss_SG(optimal.x, c1, confirmed, death, hosp, visits, n_0, SEIARG_SG)
	...
	result, [S, E, I, A, IH, IN, D, R, G, H, betas] \
		= simulate_SG(size, SEIARG_SG, S, E, I, A, IH, IN, D, R, G, H, beta, gammaE, alpha, gamma, gamma2, gamma3,
		              a1, a2, a3, h, eta, c1, n_0, H0)
	...
	return [beta, gammaE, alpha, gamma, gamma2, gamma3, a1, a2, a3, h, eta, c1, metric1, metric2, metric3, r1, r2,
	        r3], min_loss
...
def fit_init(confirmed0, death0, hosp0, days, reopen_day, n_0, metric1, metric2, metric3):
	...
	result, [S, I, IH, IN, D, R, G, H, betas] \
		= simulate_SG(size, SIARG, S, I, IH, IN, D, R, G, H, beta, gamma, gamma2, a1, a2, a3, h, k, k2,
...
def fit_state(state, ConfirmFile, DeathFile, PopFile, VisitsFile):
	t1 = time.perf_counter()
	print('Fitting', state)

	if not os.path.exists(f'Safe Graph/Fitting_{end_date}/{state}'):
		os.makedirs(f'Safe Graph/Fitting_{end_date}/{state}')
	...
	# select visit data
	df = pd.read_csv(VisitsFile)
	visits = df[df.iloc[:, 0] == state]
	visits = visits.iloc[0, 1:]
	# print(visits)
	visits = visits.rolling(7).mean()
	visits = visits.loc[start_date: end_date]

	# fitting
	para = MT_SG(confirmed, death, hosp, visits, n_0, days)

	fig_file = f'Safe Graph/Fitting_{end_date}/{state}/sim.png'
	[S, E, I, A, IH, IN, D, R, G, H, betas] = plot_SG(fig_file, state, confirmed, death, hosp, visits, days, n_0, para)

	csv_file = f'Safe Graph/Fitting_{end_date}/{state}/sim.csv'
	save_sim_SG(csv_file, [S, E, I, A, IH, IN, D, R, G, H, betas], days)

	para_file = f'Safe Graph/Fitting_{end_date}/{state}/para.csv'
	save_para_SG(para_file, [para])
...
# plot result
def plot_SG(fig_file, state, confirmed, death, hosp, visits, days, n_0, para):
	[beta, gammaE, alpha, gamma, gamma2, gamma3, a1, a2, a3, h, eta, c1, metric1, metric2, metric3, r1, r2, r3] = para
	para_label = ['beta', 'gammaE', 'alpha', 'gamma', 'gamma2', 'gamma3', 'a1', 'a2', 'a3', 'h', 'eta', 'c1',
	              'metric1', 'metric2', 'metric3', 'r1', 'r2', 'r3']
	...
	ax.plot(days, [i / 1000 for i in H], label='Release')
...
# combined fitting
def MT_SG(confirmed, death, hosp, visits, n_0, days):
	para_best = []
	min_loss = 10000
	with concurrent.futures.ProcessPoolExecutor() as executor:
		t1 = time.perf_counter()
		results = [executor.submit(fit_SG, confirmed, death, hosp, visits, days, n_0, 0, 0, 0) for _ in
		           range(num_threads)]
...
def CI_combined(confirmed, death, hosp, n_0, days, reopen_day, metric1, metric2, metric3):
	...
	with concurrent.futures.ProcessPoolExecutor() as executor:
		t1 = time.perf_counter()
		results = [
			executor.submit(fit_SG, confirmed, death, hosp, days, reopen_day, n_0, metric1, metric2, metric3) for
			_ in range(num_CI)]
...
def fit_all():
	...
	# states = ['AZ-Maricopa', 'CA-Los Angeles', 'CA-Riverside', 'FL-Broward', 'FL-Miami-Dade', 'GA-Fulton', 'IL-Cook',
	#           'LA-Jefferson', 'MA-Middlesex', 'MD-Prince George\'s', 'MN-Hennepin', 'NC-Mecklenburg', 'NJ-Bergen',
	#           'NJ-Hudson', 'NV-Clark', 'NY-New York', 'OH-Franklin', 'PA-Philadelphia', 'TN-Shelby', 'TX-Dallas',
	#           'TX-Harris', 'UT-Salt Lake', 'VA-Fairfax', 'WI-Milwaukee']
	#
	states = ['FL-Miami-Dade']

	ConfirmFile = 'JHU/JHU_Confirmed-counties.csv'
	DeathFile = 'JHU/JHU_Death-counties.csv'
	PopFile = 'JHU/CountyPopulation.csv'
	VisitsFile = 'Safe Graph/total visits.csv'

	for i in range(len(states)):
		fit_state(states[i], ConfirmFile, DeathFile, PopFile, VisitsFile)

	# state = 'NY-New York'
	# dates = ['2020-06-22', end_date]
	# fit_state(state, 'JHU/JHU_Confirmed-counties.csv', 'JHU/JHU_Death-counties.csv',
...
def test():
	df = pd.read_csv('Safe Graph/total visits.csv')
	cols = df.columns
	start_dt = datetime.datetime(2020, 1, 1)
	print(start_dt)
	dts = [start_dt + datetime.timedelta(i) for i in range(len(cols) - 1)]
...
def interval(state, reopen_date, ax):
	...
	for i in range(len(df)):
		...
		result, [S, I, IH, IN, D, R, G, H, betas] \
			= simulate_SG(size, SIARG, S2, I2, IH2, IN2, D2, R2, G2, H2, beta, gamma, gamma2, a1, a2, a3,
