...
from SIRfunctions import SIRG_sd, SIRG, computeBeta, SIRG_new
import datetime
...
# simulate the initial phase
def simulate1(size, SIRG, S, I, IH, IN, D, R, G, beta, gamma, gamma2, a1, a2, a3, eta, c1, n_0):
	result = True
	for i in range(1, size):
		delta = SIRG(i, [S[i - 1], I[i - 1], IH[i - 1], IN[i - 1], D[i - 1], R[i - 1], G[i - 1], beta, gamma, gamma2,
		                 a1, a2, a3, eta, n_0, c1])
		S.append(S[-1] + delta[0])
		I.append(I[-1] + delta[1])
		...
		if S[-1] < 0:
			result = False
			break
	return result, [S, I, IH, IN, D, R, G]


# simulate the reopen phase
def simulate2(size, SIRG, S, I, IH, IN, D, R, G, H, beta, gamma, gamma2, a1, a2, a3, h, k, k2, eta, c1, n_0):
	result = True
	eta2 = eta
	for i in range(1, size):
		if i <= release_duration:
			S[-1] += h
			H[-1] -= h
		...
		# if change_eta2:
		# 	eta2 += release / n_0
		kk = 1 + min(k_drop, i) / k_drop * (k - 1)
		kk2 = 1 + min(k_drop, i) / k_drop * (k2 - 1)
		# kk = 1
		# kk2 = 1
		delta = SIRG(i, [S[i - 1], I[i - 1], IH[i - 1], IN[i - 1], D[i - 1], R[i - 1], G[i - 1], beta, gamma,
		                 gamma2, kk * a1, kk2 * a2, a3, eta2, n_0, c1])
		S.append(S[-1] + delta[0])
		I.append(I[-1] + delta[1])
		IH.append(IH[-1] + delta[2])
		IN.append(IN[-1] + delta[3])
		...
		if S[-1] < 0:
			result = False
			break
	return result, [S, I, IH, IN, D, R, G, H]
...
def loss1(point, c1, confirmed, death, hosp, n_0, SIRG):
	...
	R = [0]
	G = [confirmed[0]]
	result, [S, I, IH, IN, D, R, G] = simulate1(size, SIRG, S, I, IH, IN, D, R, G, beta, gamma, gamma2, a1, a2, a3, eta,
	                                            c1, n_0)
...
def loss2(point, para_init, confirmed, death, hosp, start_point, n_0, SIRG):
	...
	k = point[2]
	k2 = point[3]
	[beta, gamma, gamma2, a1, a2, a3, eta, c1, metric1, metric2, metric3] = para_init
	size = len(confirmed)
	...
	R = [R0]
	G = [G0]
	H = [h * release_duration]
	# H = [Hiding_init * eta * n_0]

	result, [S, I, IH, IN, D, R, G, H] = simulate2(size, SIRG, S, I, IH, IN, D, R, G, H, beta, gamma, gamma2, a1, a2,
	                                               a3, h, k, k2, eta, c1, n_0)
	...
	metric1 = r2_score(weighted_death, weighted_D)
	metric2 = r2_score(weighted_hosp, weighted_IH)
	return -(theta2 * metric0 + 1 * (1 - theta2) / 2 * metric1 + 1 * (1 - theta2) / 2 * metric2) + Hiding_init / 1000000
...
def fit_init(confirmed0, death0, hosp0, n_0, metric1, metric2, metric3):
	...
	for c1 in np.arange(c1_range[0], c1_range[1], 0.01):
		...
		optimal = minimize(loss1, [uni(beta_range[0], beta_range[1]),
		                           ...
		                           uni(eta_range[0], eta_range[1])],
		                   args=(c1, confirmed, death, hosp, n_0, SIRG_new), method='L-BFGS-B',
		                   bounds=[beta_range, gamma_range, gamma2_range, a1_range, a2_range, a3_range, eta_range])
		current_loss = loss1(optimal.x, c1, confirmed, death, hosp, n_0, SIRG_new)
	...
	result, [S, I, IH, IN, D, R, G] = simulate1(size, SIRG_new, S, I, IH, IN, D, R, G, beta, gamma, gamma2, a1, a2, a3,
	                                            eta, c1, n_0)
	...
	return [beta, gamma, gamma2, a1, a2, a3, eta, c1, metric1, metric2, metric3], min_loss
...
def fit_reopen(confirmed0, death0, hosp0, n_0, para_init, start_point, metric1, metric2, metric3):
	...
	hosp = hosp0.copy()
	size = len(confirmed)
	[beta, gamma, gamma2, a1, a2, a3, eta, c1, m1, m2, m3] = para_init
	if metric2 != 0 or metric1 != 0:
		scale1 = pd.Series(np.random.normal(1, metric1, size))
		confirmed = [max(confirmed[i] * scale1[i], 1) for i in range(size)]
		scale2 = pd.Series(np.random.normal(1, metric2, size))
	...
	optimal = minimize(loss2, [uni(0, 0.5 * eta * n_0),
	                   ...
	                   args=(para_init, confirmed, death, hosp, start_point, n_0, SIRG_new),
	                   method='L-BFGS-B', bounds=[(0, 5 * eta * n_0), (0, 5), k_range, k2_range])
	min_loss = loss2(optimal.x, para_init, confirmed, death, hosp, start_point, n_0, SIRG_new)
	h = optimal.x[0]
	...
	result, [S, I, IH, IN, D, R, G, H] = simulate2(size, SIRG_new, S, I, IH, IN, D, R, G, H, beta, gamma, gamma2, a1,
	                                               a2, a3, h, k, k2, eta, c1, n_0)

	if len(G) != size:
		return [h, Hiding_init, k, k2, metric1, metric2, metric3], 1000000
	# print(f'size={size}, {len(confirmed)}, {len(G)}, {len(death)}, {len(D)}, {len(hosp)}, {len(IH)}')
	data1 = [(confirmed[i] - G[i]) / confirmed[i] for i in range(size)]
	data2 = [(death[i] - D[i]) / death[i] for i in range(size)]
	data3 = [(hosp[i] - IH[i]) / hosp[i] for i in range(size)]
	...
	return [h, Hiding_init, k, k2, metric1, metric2, metric3], min_loss
	...
	with concurrent.futures.ProcessPoolExecutor() as executor:
		...
		results = [
			executor.submit(fit_reopen, confirmed_reopen, death_reopen, hosp_reopen, n_0, para_init, start_point, 0, 0,
			                0) for _ in range(num_threads_reopen)]
		...
		for f in concurrent.futures.as_completed(results):
			...
			if current_loss < min_loss:
				...
				print(f'best paras updated at {threads}')
		# if k % 10 == 0:
...
def MT_reopen_dist(state, confirmed_reopen, death_reopen, hosp_reopen, n_0, para_init, start_point, para_reopen):
	[h, Hiding_init, k, k2, metric1, metric2, metric3] = para_reopen
	paras = [para_reopen]  # store the bet fit parameters in the first row
	with concurrent.futures.ProcessPoolExecutor() as executor:
		t1 = time.perf_counter()
		results = [executor.submit(fit_reopen, confirmed_reopen, death_reopen, hosp_reopen, n_0, para_init, start_point,
		...
		for f in concurrent.futures.as_completed(results):
			...
			threads += 1
			if threads % 100 == 0:
				print(f'{threads}/{num_threads_dist_reopen} thread(s) completed')
...
def MT_init_dist(state, confirmed_init, death_init, hosp_init, n_0, para_init):
	...
	with concurrent.futures.ProcessPoolExecutor() as executor:
		...
		for f in concurrent.futures.as_completed(results):
			...
			threads += 1
			if threads % 100 == 0:
				print(f'{threads}/{num_threads_dist} thread(s) completed')
...
def save_para_reopen(state, paras):
	para_label = ['h', 'Hiding_init', 'k', 'k2', 'metric1', 'metric2', 'metric3']
...
def save_para_init(state, paras):
	para_label = ['beta', 'gamma', 'gamma2', 'a1', 'a2', 'a3', 'eta', 'c1', 'metric1', 'metric2', 'metric3']
	df = pd.DataFrame(paras, columns=para_label)
...
def plot_init(plot_filename, confirmed, death, hosp, days, n_0, para):
	size = len(confirmed)
	[beta, gamma, gamma2, a1, a2, a3, eta, c1, metric1, metric2, metric3] = para
	S = [n_0 * eta]
	I = [confirmed[0]]
	IH = [I[-1] * gamma]
	IN = [I[-1] * gamma2]
	...
	# H = [Hiding * n_0 * eta]
	# Betas = [beta]
	result, [S, I, IH, IN, D, R, G] = simulate1(size, SIRG_new, S, I, IH, IN, D, R, G, beta, gamma, gamma2, a1, a2, a3, eta,
	                                            c1, n_0)
	# for i in range(1, size):
	# 	delta = SIRG_new(i, [S[i - 1], I[i - 1], IH[i - 1], IN[i - 1], D[i - 1], R[i - 1], G[i - 1], beta, gamma,
	# 	                     gamma2, a1, a2, a3, eta, n_0, c1])
	# 	S.append(S[-1] + delta[0])
	# 	I.append(I[-1] + delta[1])
	# 	IH.append(IH[-1] + delta[2])
	# 	IN.append(IN[-1] + delta[3])
...
def plot_reopen(plot_filename, confirmed_reopen, death_reopen, hosp_reopen, n_0, para_init, start_point, days_reopen,
                para_reopen):
	...
	H = [h * release_duration]
	# H = [Hiding_init * eta * n_0]
	result, [S, I, IH, IN, D, R, G, H] = simulate2(size, SIRG_new, S, I, IH, IN, D, R, G, H, beta, gamma, gamma2, a1, a2,
	                                               a3, h, k, k2, eta, c1, n_0)
	# eta2 = eta
	# for i in range(1, size):
	# 	# if i < 40:
	...
	# 	if change_eta2:
	# 		eta2 += release / n_0
	# 	k2 = 1 + min(14, i) / 14 * (k - 1)
	# 	delta = SIRG_new(i, [S[i - 1], I[i - 1], IH[i - 1], IN[i - 1], D[i - 1], R[i - 1], G[i - 1], beta, gamma,
	# 	                     gamma2, a1, k2 * a2, a3, eta2, n_0, c1])
	# 	S.append(S[-1] + delta[0])
	# 	I.append(I[-1] + delta[1])
	# 	IH.append(IH[-1] + delta[2])
	# 	IN.append(IN[-1] + delta[3])
	...
	if plot_filename != '':
		...
		# ax2.plot(days_reopen, [i / 1000 for i in IN], label='IN')
		ax.plot(days_reopen, [i / 1000 for i in H], label='H')
		# ax.plot(days_reopen, [i / 1000 for i in S], label='S')
		ax.plot(days_reopen, [i / 1000 for i in I], label='I')
		ax.legend()
		ax2.legend()
		fig.autofmt_xdate()
		plt.savefig(plot_filename, bbox_inches="tight")
