...
def loss_init(point, c1, confirmed, death, n_0, vaccine_day):
	size = len(confirmed)
	beta = point[0]
	gammaE = point[1]
	alpha = point[2]
	...
	result, [S, E, I, A, IH, IN, D, R, G, H,
	         ...
	         betas] = simulate_vac(size, S, E, I, A, IH, IN, D, R, G, H, beta, gammaE, alpha, gamma, gamma2, gamma3, a1,
	                               a2, a3, 1, 0, eta, c1, n_0, size, vaccine_day, daily_vspeed,
...
def loss_reopen(point, confirmed, death, n_0, vaccine_day, reopen_day, para_init):
	size = len(confirmed)
	h = point[0]
	Hiding_init = point[1]
	[beta, gammaE, alpha, gamma, gamma2, gamma3, a1, a2, a3, eta, c1, E_initial, I_initial] = para_init
	S = [n_0 * eta]
	E = [n_0 * eta * E_initial]
	I = [n_0 * eta * I_initial * (1 - alpha)]
	A = [n_0 * eta * I_initial * alpha]
	...
	result, [S, E, I, A, IH, IN, D, R, G, H,
		...
		simulate_vac(size, S, E, I, A, IH, IN, D, R, G, H, beta, gammaE, alpha, gamma, gamma2, gamma3, a1, a2, a3, h,
		             Hiding_init, eta, c1, n_0, reopen_day, vaccine_day, daily_vspeed, v_period1, v_period2, v_eff1,
		             v_eff2)
...
def fit_init(confirmed0, death0, vaccine_day, n_0):
	np.random.seed()
	...
	for c1 in np.arange(c1_range[0], c1_range[1], 0.01):
		...
		optimal = minimize(loss_init, [uni(beta_range[0], beta_range[1]),
		                   ...
		                   args=(c1, confirmed, death, n_0, vaccine_day), method='L-BFGS-B',
		                   bounds=[beta_range,
		                           gammaE_range,
		                           alpha_range,
		                           gamma_range,
		                           ...
		                           E_initial_range,
		                           I_initial_range])
		current_loss = loss_init(optimal.x, c1, confirmed, death, n_0, vaccine_day)
	...
	result, [S, E, I, A, IH, IN, D, R, G, H,
	         S0, E0, I0, A0, IH0, IN0, D0, R0, G0, H0, S1, E1, I1, A1, IH1, IN1, D1, R1, G1, H1,
	         S2, E2, I2, A2, IH2, IN2, D2, R2, G2, H2, S3, E3, I3, A3, IH3, IN3, D3, R3, G3, H3, betas] = \
		simulate_vac(size, S, E, I, A, IH, IN, D, R, G, H, beta, gammaE, alpha, gamma, gamma2, gamma3, a1, a2, a3, 1, 0,
		             eta, c1, n_0, size, vaccine_day, daily_vspeed, v_period1, v_period2, v_eff1, v_eff2)
	...
	return [beta, gammaE, alpha, gamma, gamma2, gamma3, a1, a2, a3, eta, c1, E_initial, I_initial], min_loss


def fit_reopen(confirmed0, death0, vaccine_day, reopen_day_gov, n_0, para_init):
	np.random.seed()
	confirmed = confirmed0.copy()
	death = death0.copy()
	size = len(confirmed)
	...
	c_max = 0
	min_loss = 10000
	for reopen_day in range(reopen_day_gov, reopen_day_gov + 15):
		optimal = minimize(loss_reopen, [uni(h_range[0], h_range[1]),
		                                 uni(Hiding_init_range[0], Hiding_init_range[1])],
		                   args=(confirmed, death, n_0, vaccine_day, reopen_day, para_init), method='L-BFGS-B',
		                   bounds=[h_range, Hiding_init_range])
		current_loss = loss_reopen(optimal.x, confirmed, death, n_0, vaccine_day, reopen_day, para_init)
		if current_loss < min_loss:
			# print(f'updating loss={current_loss} with c1={c1}')
			min_loss = current_loss
			h = optimal.x[0]
			Hiding_init = optimal.x[1]
			reopen_day_best = reopen_day

	reopen_day = reopen_day_best
	S = [n_0 * eta]
	E = [n_0 * eta * E_initial]
	I = [n_0 * eta * I_initial * (1 - alpha)]
	A = [n_0 * eta * I_initial * alpha]
	...
	return [h, Hiding_init, metric1, metric2, r1, r2, reopen_day], min_loss


def MT_fitting_init(state, confirmed, death, n_0, vaccine_day):
	para_best = []
	min_loss = 10000
	with concurrent.futures.ProcessPoolExecutor() as executor:
		# t1 = time.perf_counter()
		results = [executor.submit(fit_init, confirmed, death, vaccine_day, n_0) for _ in range(num_threads)]
...
def MT_fitting_reopen(state, confirmed, death, n_0, vaccine_day, reopen_day_gov, para_init):
	para_best = []
	min_loss = 10000
	with concurrent.futures.ProcessPoolExecutor() as executor:
		# t1 = time.perf_counter()
		results = [executor.submit(fit_reopen, confirmed, death, vaccine_day, reopen_day_gov, n_0, para_init) for _ in
		           range(num_threads)]
...
def fit_state_split(state, ConfirmFile, DeathFile, PopFile, end_date):
	t1 = time.perf_counter()
	state_path = f'UK/fitting_split_{end_date}_{reopen_date}/{state}'
	...
	days_init = days[days.index(start_date):days.index(reopen_date) + 1]
	...
	confirmed_init = confirmed.iloc[0].loc[start_date: reopen_date]
	death_init = death.iloc[0].loc[start_date: reopen_date]
	for i in range(len(death)):
		if death_init.iloc[i] == 0:
			death_init.iloc[i] = 0.01
	death_init = death_init.tolist()

	reopen_day_gov = days_full.index(datetime.datetime.strptime(reopen_date, '%Y-%m-%d'))
	vaccine_day = days_full.index(datetime.datetime.strptime(vac_date, '%Y-%m-%d'))

	# initial fitting
	para_init = MT_fitting_init(state, confirmed_init, death_init, n_0, vaccine_day)

	# reopen fitting
	para_reopen = MT_fitting_reopen(state, confirmed_full, death_full, n_0, vaccine_day, reopen_day_gov, para_init)
	...
	[S, E, I, A, IH, IN, D, R, G, H,
	 S0, E0, I0, A0, IH0, IN0, D0, R0, G0, H0, S1, E1, I1, A1, IH1, IN1, D1, R1, G1, H1,
	 S2, E2, I2, A2, IH2, IN2, D2, R2, G2, H2, S3, E3, I3, A3, IH3, IN3, D3, R3, G3, H3, betas] = \
		plot_full(state, confirmed_full, death_full, days_full, n_0, para, state_path, vaccine_day)
...
def save_para_vac(paras, state_path):
	para_label = ['beta', 'gammaE', 'alpha', 'gamma', 'gamma2', 'gamma3', 'a1', 'a2', 'a3', 'eta', 'c1', 'E_initial',
	              'I_initial', 'h', 'Hiding_init', 'metric1', 'metric2', 'r1', 'r2', 'reopen']
...
def plot_full(state, confirmed, death, days, n_0, para, state_path, vaccine_day):
	[beta, gammaE, alpha, gamma, gamma2, gamma3, a1, a2, a3, eta, c1, E_initial, I_initial, h, Hiding_init, metric1,
...
def fit_all_split(end_date):
	...
	matplotlib.use('Agg')
	with concurrent.futures.ProcessPoolExecutor() as executor:
		[executor.submit(fit_state_split, state, 'UK/UK_confirmed.csv', 'UK/UK_death.csv', 'UK/state_population.csv',
		                 end_date) for state in states]

	# for state in states[:1]:
	# 	fit_state_split(state, 'UK/UK_confirmed.csv', 'UK/UK_death.csv', 'UK/state_population.csv', end_date)
...
def extend_state_split(state, ConfirmFile, DeathFile, PopFile, ParaFile, sim_enddate, data_enddate):
	state_path = f'UK/extended_split_{sim_enddate}/{state}'
	if not os.path.exists(state_path):
		os.makedirs(state_path)
	df = pd.read_csv(ParaFile)
	beta, gammaE, alpha, gamma, gamma2, gamma3, a1, a2, a3, eta, c1, E_initial, I_initial, h, Hiding_init, metric1, metric2, r1, r2, reopen_date = \
		df.iloc[0]
	...
	ax.set_ylabel('Cumulative Cases (Thousands)')
	ax.axvline(days[sim_endday], linestyle='dashed', color='tab:red', label=sim_enddate)
	ax.plot(days, [i / 1000 for i in confirmed], linewidth=5, linestyle=':', label="Reported")
	ax.plot(days, [i / 1000 for i in G], label='Simulated')
	...
	fig.autofmt_xdate()
	fig.savefig(f'{state_path}/{state}_ext.png', bbox_inches="tight")
	plt.close(fig)
	return G, D, dG, dD, confirmed, death, d_confirmed, d_death, days


def extend_all_split(sim_enddate):
	matplotlib.use('Agg')

	# sim_enddate = '2021-06-15'
	data_enddate = '2021-07-03'
	sim_releasedate = '2021-04-25'
	sim_folder = f'UK/fitting_split_{sim_enddate}_{sim_releasedate}'
	# with concurrent.futures.ProcessPoolExecutor() as executor:
	...
	for state in states:
		G, D, dG, dD, confirmed, death, d_confirmed, d_death, days = \
			extend_state_split(state, 'UK/UK_confirmed.csv', 'UK/UK_death.csv', 'UK/state_population.csv',
	...
	if sim_endday + 7 <= data_endday:
		...
		error_ratio /= 7
		print(f'UK  7-day new case error ratio from {sim_enddate} = {round(error_ratio, 6)}')

		error_ratio = 0
		MA_UK_G = moving_avg(UK_G, 7)
		MA_UK_confirmed = moving_avg(UK_confirmed, 7)
		for i in range(sim_endday + 1, sim_endday + 8):
			error_ratio += (abs(MA_UK_G[i] - MA_UK_confirmed[i]) / MA_UK_confirmed[i])
		error_ratio /= 7
		print(f'UK  7-day cumulative error ratio from {sim_enddate} = {round(error_ratio, 6)}')
	...
	if sim_endday + 14 <= data_endday:
		...
		for i in range(sim_endday + 1, sim_endday + 15):
			error_ratio += (abs(MA_UK_dG[i] - MA_UK_d_confirmed[i]) / MA_UK_d_confirmed[i])
		error_ratio /= 14
		print(f'UK 14-day new case error ratio from {sim_enddate} = {round(error_ratio, 6)}')
		...
		for i in range(sim_endday + 1, sim_endday + 15):
			error_ratio += (abs(MA_UK_G[i] - MA_UK_confirmed[i]) / MA_UK_confirmed[i])
		error_ratio /= 14
		print(f'UK 14-day cumulative error ratio from {sim_enddate} = {round(error_ratio, 6)}')
	...
	ax.set_ylabel('Cumulative Cases (Thousands)')
	ax.axvline(days[sim_endday], linestyle='dashed', color='tab:red', label=sim_enddate)
	ax.plot(days, [i / 1000 for i in UK_confirmed], linewidth=5, linestyle=':', label="Reported")
	ax.plot(days, [i / 1000 for i in UK_G], label='Simulated')
	...
	fig.autofmt_xdate()
	fig.savefig(f'UK/extended_split_{sim_enddate}/UK_ext.png', bbox_inches="tight")
	plt.close(fig)

	return


def save_para_all_vac(end_date):
...
def main():
	# fit_all_split('2021-06-08')
	# fit_all_split('2021-06-15')
	# fit_all_split('2021-06-22')
	# save_para_all_vac()
	extend_all_split('2021-06-08')
	extend_all_split('2021-06-15')
	extend_all_split('2021-06-22')
	# tmp()
	return
