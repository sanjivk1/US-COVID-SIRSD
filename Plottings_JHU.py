...
def plot_bar():
	...
	state = 'NY-New York'
	dates = ['2020-06-22', end_date]
	point = read_bar(state, f'JHU/combined2W_{end_date}/{state}', 'JHU/JHU_Confirmed-counties.csv',
	                 'JHU/CountyPopulation.csv', dates)
	points.append(point)
	...
	for i in range(len(points)):
		...
		ax2.bar3d(np.log(points[i][0]) if log_axis else points[i][0],
		          ...
		          points[i][3],
		          alpha=0.6, color=color_dict.setdefault(state, 'black'))
	fig.savefig('JHU/3D/3D.png', bbox_inches="tight")
	plt.close(fig)
	...
	fig.savefig('JHU/3D/3D_log.png', bbox_inches="tight")
	# plt.show()
	plt.close(fig)
	...
	ax.legend()
	fig.savefig('JHU/3D/log_S_left.png', bbox_inches="tight")
	# plt.show()
	plt.close(fig)
	...
	ax.set_ylabel('log(PIR)')
	ax.legend()
	fig.savefig('JHU/3D/log_I.png', bbox_inches="tight")
	# plt.show()
	plt.close(fig)
...
def peak_by_H():
	...
	ax.set_ylim(yl, yh)
	ax.view_init(elev=5, azim=315)
	fig.savefig('JHU/3D/3D_H.png', bbox_inches="tight")
	# plt.show()
	plt.close(fig)
	...
	ax.set_ylabel('log(PIR)')
	ax.legend()
	fig.savefig('JHU/3D/log_H.png', bbox_inches="tight")
	# plt.show()
	plt.close(fig)
